...
/* FAST ADVERTISING: 20-30ms intervals for quick phone connection */
#define ADV_INTERVAL_MIN                    (0x0020)  /* 20ms = 32 * 0.625ms (was 0x0080 = 80ms) */
#define ADV_INTERVAL_MAX                    (0x0030)  /* 30ms = 48 * 0.625ms (was 0x00A0 = 100ms) */
...
/**
 * CRITICAL: Define number of simultaneous connections (phone + peer)
 * Must be defined BEFORE CFG_BLE_NUM_RADIO_TASKS
 * Set to 4: phone_conn(1M) + peer_conn(Coded S=8) + scan_window + adv_slot
 * With Coded S=8 each packet takes 8x airtime - need extra slots for scheduler.
 */
 ...
 */
#define CFG_BLE_NUM_GATT_ATTRIBUTES                     (48)  /* Sensor(25) + Motor(3) + BConnection(7) + Chat(6) = 41 used; margin for future.
                                                              * Was 30: too small -> Chat Server aci_gatt_srv_add_service failed with
                                                              * 0x87 BLE_STATUS_OUT_OF_MEMORY and chat service never existed in GATT DB. */
 ...
 * supported links.
 * INCREASED to 150 to handle dual connection (phone + peer) with multiple subscriptions.
 * Dual connection requires: Phone (8 subscriptions) + Peer (5 subscriptions) = 13 active channels
 * Each notification uses 1-2 MBLOCKS, peak usage ~26-36 MBLOCKS per cycle.
 * Base MBLOCKS ~30, +150 margin = ~180 total (3x safety margin for dual connection)
 * This prevents 0x0C (INSUFFICIENT_RESOURCES) errors during dual operation.
 * Previous value of 110 was insufficient - still saw 0x0C errors in testing.
 */
#define CFG_BLE_MBLOCK_COUNT_MARGIN                     (150)
...
typedef enum
{
  ...
  START_SCAN,
  UPDATE_BLST,
  TASK_CONNECT_REQUEST,
  TASK_INIT_SENSORS,
  TASK_CLASSIFY_CONNECTION,  /* NEW: Classify incoming connection as phone or peer */
